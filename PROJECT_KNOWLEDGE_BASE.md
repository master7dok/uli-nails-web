...
## 4. Авторизація в Адмін-панелі

### Яка змінна відповідає за пароль?
- **`ADMIN_PASSWORD`** (в `.env` або налаштуваннях середовища Coolify).
- Якщо змінна не задана, використовується резервний пароль: `uliana_admin_secret`.

### Як влаштовано механізм перевірки (`src/lib/auth.ts`):
- Пароль може бути в лапках або без (`ADMIN_PASSWORD="secret"` або `ADMIN_PASSWORD=secret`). Код автоматично видаляє зайві лапки та пробіли.
- При успішному вході створюється токен (Base64 від пароля), який записується:
  1. У cookie браузера `uli_admin_token` (HTTP-only).
  2. У локальний заголовок `Authorization: Bearer <token>`.
  3. У кастомний заголовок `x-admin-token`.
- Завдяки такій потрійній перевірці запити в Coolify та через Nginx/Traefik проходять стабільно, навіть якщо проксі-сервер обрізає cookies або headers.
